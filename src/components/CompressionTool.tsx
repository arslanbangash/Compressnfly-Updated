...
const CompressionTool = () => {
  ...
  const getFileIcon = (file: File) => {
    ...
    if (
      ...
      file.name.toLowerCase().includes(".rtf") ||
    ...
    if (
      ...
      file.name.toLowerCase().includes(".7z") ||
      file.name.toLowerCase().includes(".tar") ||
      file.name.toLowerCase().includes(".gz")
    )
      return Archive;
    if (
  ...
  const compressFiles = async () => {
    ...
    const getCompressionRatio = (file: File) => {
      ...
      const isSpreadsheet =
        ...
        file.name.toLowerCase().includes(".xls") ||
        file.name.toLowerCase().includes(".csv") ||
        file.name.toLowerCase().includes(".ods");
      ...
      if (isVideo) {
        return compressionLevel === "high-quality"
          ? 0.85
          : compressionLevel === "web-optimized"
            ? 0.7
            : compressionLevel === "balanced"
              ? 0.6
              : 0.45; // More aggressive for video
      } else if (isAudio) {
        return compressionLevel === "high-quality"
          ? 0.88
          : compressionLevel === "web-optimized"
        ...
        return compressionLevel === "high-quality"
          ...
          : compressionLevel === "web-optimized"
            ...
            : compressionLevel === "balanced"
              ? 0.75
              : 0.6; // Original ratios for images
        ...
        return compressionLevel === "high-quality"
          ...
          : compressionLevel === "web-optimized"
            ...
            : compressionLevel === "balanced"
              ? 0.82
              : 0.7; // Good compression for office docs
      } else if (isArchive) {
        return compressionLevel === "high-quality"
          ? 0.95
          : compressionLevel === "web-optimized"
            ...
            : compressionLevel === "balanced"
              ? 0.85
              : 0.75; // Minimal compression for archives
      } else if (isText) {
        return compressionLevel === "high-quality"
          ? 0.88
          : compressionLevel === "web-optimized"
            ...
            : compressionLevel === "balanced"
              ? 0.7
              : 0.55; // Good compression for text files
      } else {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      setFiles((prev) =>
        prev.map((f, index) => (index === i ? { ...f, compressedSize } : f)),
      );
    ...
    toast({
      ...
      description:
        "Your files have been successfully compressed with optimized quality.",
    });
  };
  ...
  const downloadAsZip = async () => {
    ...
    files.forEach((fileInfo) => {
      const compressedName = fileInfo.file.name.replace(
        /(\.[\w\d_-]+)$/i,
        "_compressed$1",
      );
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="web-optimized">
                        Web Optimized 80%
                      </SelectItem>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <h4 className="font-medium text-foreground">
                        File{files.length > 1 ? "s" : ""} compressed
                        successfully!
                      </h4>
                      <p className="text-sm text-muted-foreground">
                        Reduced from {formatFileSize(totalOriginalSize)} to{" "}
              ...
              <div className="space-y-6 mb-6">
                {files.map((fileInfo, index) => {
                  ...
                  const compressionPercent = fileInfo.compressedSize
                    ? Math.round(
                        ((fileInfo.file.size - fileInfo.compressedSize) /
                          fileInfo.file.size) *
                          100,
                      )
                    : 0;
                    ...
                    <div key={index} className="bg-muted rounded-lg p-6">
                      <div className="flex items-center mb-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {formatFileSize(fileInfo.file.size)} →{" "}
                            {formatFileSize(
                              fileInfo.compressedSize || fileInfo.file.size,
                            )}
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div className="bg-background rounded-lg p-4">
                          ...
                          <p className="text-xs font-medium text-center text-success">
                            {formatFileSize(
                              fileInfo.compressedSize || fileInfo.file.size,
                            )}
                          </p>
